...
.sheet{width:89mm;height:127mm;background:#fff;position:relative;overflow:hidden;font-family:'Zen Kaku Gothic New',sans-serif;color:var(--ink);}
/* Trim box = finished name-card size. Cut along this rectangle. The card art bleeds
   PAST this line (full bleed) so a slight cut drift never exposes a white edge. */
.slot{position:absolute;left:2mm;width:85mm;height:51.4mm;border:.2mm dashed var(--guide);}
.slot.top{top:8.6mm;}
...
.card{position:absolute;top:50%;left:50%;width:91mm;height:55mm;
  /* Full bleed: scale so the art (~89.6mm wide) overruns the 85mm trim box by ~2mm
     on every side, reaching the L判 edge. The sheet clips the overrun.
     translate + scale MUST be one transform (a separate scale property composes in a different order and offsets the card). */
  transform:translate(-50%,-50%) scale(.985);transform-origin:center;
  overflow:hidden;border-radius:0;display:flex;align-items:stretch;
  background:
